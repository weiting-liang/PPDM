...
def make_sketch(outdir, kmer, sketch):
    ...
    with open(outlogfile, "wb") as log_file:
        try:
            ...
            #lwt add: write input files to a temporary file
            temp_input_file = os.path.join(outdir, 'temp_input_list.txt')
            with open(temp_input_file, 'w') as temp_file:
                for inp_file in inp_files:
                    temp_file.write(f"{inp_file}\n")

            #lwt adjust -a -l
            cmd_mash = f"mash sketch -o {outfile} -k {kmer} -p 1 -s {sketch} -a -l {temp_input_file}"
            p = Popen(cmd_mash, shell=True, stdout=PIPE, stderr=PIPE)
            stdout, stderr = p.communicate()
            log_file.write(stderr)

            #lwt add: Clean up temporary file
